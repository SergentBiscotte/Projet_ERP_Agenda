...
class AppointmentReportWizard(models.TransientModel):
    _name = "erpprojet.appointment.report.wizard"
    ...
    patient_id = fields.Many2one('erpprojet.patient', string="Patient", required=True)
    ...
    def action_print_report(self):
        ...
        appointments = self.env['erpprojet.appointment'].search_read(domain)
        
        data = {
            'form_data': self.read()[0],
            'appointments': appointments
        }
        return self.env.ref('erpprojet.action_report_appointment_card').report_action(self, data=data)
